...
![Скрипт в редакторе Tampermonkey](img/tampermonkey-editor.png)

> Вставлять нужно **вместе** с блоком `// ==UserScript== … // ==/UserScript==` в начале файла
> (строки 1–13 на скриншоте). Это не комментарий, а конфигурация: там указано, на каких
> страницах скрипт запускается и какие функции хранения ему доступны. Без неё он не заработает.

Жёлтые треугольники на полях — подсказки встроенного в редактор линтера. На работу скрипта
они не влияют, реагировать на них не нужно.
...
Какая версия стоит сейчас, видно там же в редакторе — строка `@version` в начале скрипта.
Её же скрипт пишет в консоль при каждой загрузке страницы.
